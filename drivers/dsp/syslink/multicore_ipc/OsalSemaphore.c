...
OsalSemaphore_create(u32 semType)
{
	...
	if (OSALSEMAPHORE_TYPE_VALUE(semType) >= OsalSemaphore_Type_EndValue) {
		/*! @retVal NULL Invalid semaphore type (OsalSemaphore_Type)
								provided */
		...
		if (semObj == NULL) {
			/*! @retVal NULL Failed to allocate memory for semaphore
								object. */
			gt_setFailureReason (curTrace,
				GT_4CLASS,
				"OsalSemaphore_create",
...
OsalSemaphore_delete (OsalSemaphore_Handle *semHandle)
{
	...
	if (semHandle == NULL) {
		/*! @retVal OSALSEMAPHORE_E_INVALIDARG NULL provided for
			argument semHandle.*/
		...
		/*! @retVal OSALSEMAPHORE_E_HANDLE NULL Semaphore handle
								provided. */
	...
	gt_1trac(curTrace, GT_LEAVE, "OsalSemaphore_delete", status);
...
OsalSemaphore_pend(OsalSemaphore_Handle semHandle, u32 timeout)
{
	...
	if (semHandle == NULL) {
		...
		if (timeout == OSALSEMAPHORE_WAIT_NONE) {
			...
			/* TBD: Check if preempt_enable has to be used instead
								of mutex. */
			/* preempt_disable (); */
			if (semObj->value == 0u) {
				/*! @retVal OSALSEMAPHORE_E_WAITNONE WAIT_NONE
				timeout value was provided, but semaphore was
				not available. */
		...
		/* Finite and infinite timeout cases */
			...
			do {
				...
				/* TBD: Check if preempt_enable has to be used
				instead of mutex. */
				osStatus =
				mutex_lock_interruptible(&(semObj->lock));
				gt_assert(curTrace, (osStatus == 0));
				/* preempt_disable (); */
				if (semObj->value != 0) {
					if (OSALSEMAPHORE_TYPE_VALUE(semObj->semType)
					...
					/* Release the lock */
					mutex_unlock(&semObj->lock);
					/* TBD: Check if preempt_enable has to
					 * be used instead of mutex.
					 */
					/* preempt_enable (); */
					break;
				}
				...
				/* Check for status. This run-time check must
				 * remain in code and must not be optimized out.
				 */
				if (osStatus == 0) { /* Timedout? */
					/*! @retVal OSALSEMAPHORE_E_TIMEOUT
				...
				if (osStatus == -ERESTARTSYS) { /*Interrupted?*/
					/* Not a failure. Execution can be
								resumed. */
					status = -ERESTARTSYS;
					gt_setFailureReason(curTrace,
						GT_1CLASS,
				...
				if (signal_pending(current)) {
					/* Restart the operation, we don't quit
					 * as application must do a clean exit
					 */
...
OsalSemaphore_post (OsalSemaphore_Handle semHandle)
{
	...
	if (semHandle == NULL) {
		...
		if (OSALSEMAPHORE_TYPE_VALUE(semObj->semType)
			...
			osStatus = mutex_lock_interruptible(&(semObj->lock));
			gt_assert(curTrace, (osStatus == 0));
			/* TBD: Check if preempt_disable has to be used instead
								of mutex. */
			/* preempt_disable (); */
			semObj->value = 1u;
