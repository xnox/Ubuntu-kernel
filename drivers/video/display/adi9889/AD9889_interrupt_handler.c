...
DEPLINT next_segment = 0;
DEPLINT edid_block_map_offset = 0;
DEPLINT timer_count = 0;
DEPLINT hdcp_check_count = 0;
DEPLINT hdcp_nack_count = 0;
BOOL retry_edid = FALSE;
...
//	printk ("After 0x%x, 0x%x\n",interrupt_registers[0], interrupt_registers[1]);
  ...
  //process error flag
      if ((interrupt_registers[1] & 0x80))
      {
          ...
          DEPLINT error = ADI89_GET_HDCP_CONTROLLER_ERROR;
...
/*
 *Every 2 seconds this function is called.  The HDMI and HDCP leds are updated.  If
 * HDCP is supposed to be enabled, the status is checked.
 */
#if defined(HDCP_OPTION)
void timer_2_second_handler(void)
{
    ...
    if(hdcp_requested && (!hdcp_status))
    {
        ...
        { 
            hdcp_status = TRUE;
            compare_bksvs_ptr(bksv, total_bksvs, hdcp_bstatus);
        }
    }
    ...
    if(timer_count == TIMER_CALLS_PER_2_SEC)
    {
        ...
        if((hdcp_enabled)&&(!ADI89_GET_ENC_ON)&&ADI89_GET_RX_SENSE)
		{    
