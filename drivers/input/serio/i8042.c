 ...
 */
			...
			switch (data) {
				default:
					if (time_before(jiffies, last_transmit + HZ)) { //tom + HZ/10 -> HZ
						str = last_str;
						break;
					}
...
static int __devinit i8042_probe(struct platform_device *dev)
{
	...
	if (i8042_dritek)
        ...
        error=i8042_command(NULL,I8042_CMD_KBD_DISABLE);// tom +
	if(!error)
	...
	if (!i8042_noaux) {
		...
		if(!error)
			error=i8042_command(NULL,I8042_CMD_KBD_ENABLE);// tom +
		if(!error)
