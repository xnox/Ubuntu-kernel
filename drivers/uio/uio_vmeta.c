...
inline int get_current_id(kernel_share* ks,pid_t pid)
{
	int i = MAX_VMETA_INSTANCE;

	for(i=0;i<MAX_VMETA_INSTANCE;i++) {
		if(ks->user_id_list[i].pid == pid){
			return i;
		}
	}
...
int vmeta_release(struct uio_info *info, struct inode *inode)
{
	...
	printk(KERN_INFO "vmeta release current pid=0x%x, id=%d, active id=%d\n",current->pid, get_current_id(ks,current->pid),ks->active_user_id);
	down_read(&vi->sem);
	if( (ks->lock_flag==VMETA_LOCK_ON) && (get_current_id(ks,current->pid) == ks->active_user_id))
	{
		printk(KERN_ERR "vmeta error , pid(%d),uid(%d) exit abnormally\n",current->pid,ks->active_user_id);
		ks->lock_flag = VMETA_LOCK_FORCE_INIT;
		ks->ref_count--;
		vmeta_unlock(vi);
		memset(&(ks->user_id_list[ks->active_user_id]),0x0,sizeof(id_instance));
	}
