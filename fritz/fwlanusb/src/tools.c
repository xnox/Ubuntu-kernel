...
void hfree (void * mem) {
	...
	if (mem != NULL) {
		...
		if (!(FENCE1_OK(hdr, mem) && FENCE2_OK(hdr, mem))) {
			!(FENCE1_OK(hdr, mem))?(!(FENCE2_OK(hdr, mem))?
			strcpy(text, "THE BEGINNING AND END"):
			strcpy(text, "THE BEGINNING")):strcpy(text, "THE END");
			LOG(
				"FENCE AT %s OF MEMORY VIOLATED (%u bytes @ %p)!\n",
				text,
				hdr->size, 
				mem
			);
