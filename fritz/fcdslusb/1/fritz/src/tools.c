...
typedef struct __hdr {
	...
	unsigned	type;	/* TYPE_?MALLOCED */
#if !defined (NDEBUG)
#if defined (TOOLS_MEM_WATCH)
	struct __hdr *	prev;
	struct __hdr *	succ;
#endif
...
/*---------------------------------------------------------------------------*\
\*---------------------------------------------------------------------------*/
#if !defined (NDEBUG) && defined (TOOLS_MEM_WATCH)
static header_t		root = 
#if defined (TOOLS_FREE_CATCH)
	{ TYPE_NONE, NULL, NULL, 0, 0, 0x00CAFE00 };
#else
	{ TYPE_NONE, NULL, NULL,    0, 0x00CAFE00 };
#endif
#endif
...
static void *		halloc (unsigned, int, int, void *);

#define __ALLOC(s,p)	halloc((s),(p),ALLOC_NORMAL,__builtin_return_address(0))
...
int tools_init (void) {
	...
	LOG("Initializing driver tools...\n");
	assert (alloc_count == 0);
#if defined (TOOLS_MEM_WATCH)
	spin_lock_irqsave (&track_lock, flags);
	root.prev = root.succ = &root;
...
#if defined (TOOLS_MEM_WATCH)
	hdr->prev = &root;	/* Make hdr's pointers point to list */
	hdr->succ = root.succ;
	root.succ->prev = hdr;	/* Link hdr into existing list */
	root.succ = hdr;
#endif	
	spin_unlock_irqrestore (&track_lock, flags);
} /* track_alloc */
...
static inline void track_free (header_t * hdr) {
	...
	alloc_count -= hdr->size;
#if defined (TOOLS_MEM_WATCH)
	hdr->prev->succ = hdr->succ;
	hdr->succ->prev = hdr->prev;
#endif
	spin_unlock_irqrestore (&track_lock, flags);
} /* track_free */
#endif
...
#if defined (TOOLS_MEM_WATCH)
	header_t *	hp = root.succ;

	LOG("List of allocated memory blocks:\n");
	while ((hp != NULL) && (hp != &root)) {
		LOG("INUSE: %p, %u, type %c by %p\n", hp + 1, hp->size, hp->type, hp->eip);
		hp = hp->succ;
	}
#endif
	return alloc_count;
...
/*---------------------------------------------------------------------------*\
\*---------------------------------------------------------------------------*/
static void * halloc (unsigned size, int prio, int mode, void * eip) {
	unsigned	n, type;
	void *		mem;
	header_t *	hdr;
...
#if !defined (NDEBUG)
#if defined (LOG_ALLOC)
		LOG("FREE on %p, size %u, type %c, allocated by %p\n", hp + 1, hp->size, hp->type, hp->eip);
#endif
