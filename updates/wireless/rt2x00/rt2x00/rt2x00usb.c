...
static void rt2x00usb_interrupt_txdone(struct urb *urb)
{
	...
	if (!rt2x00queue_full(entry->queue))
		rt2x_ieee80211_wake_queue(rt2x00dev->hw, priv_tx->control.queue);
}
...
static void rt2x00usb_interrupt_rxdone(struct urb *urb)
{
	 ...
	 */
	header_size = rt2x_ieee80211_get_hdrlen_from_skb(entry->skb);
	if (header_size % 4 == 0) {
		    ...
		    const struct usb_device_id *id)
{
	...
	hw = rt2x_ieee80211_alloc_hw(sizeof(struct rt2x00_dev), ops->hw);
	if (!hw) {
