				 ...
				 struct ieee80211_if_sta *ifsta)
{
	...
	sta = rt2x_sta_info_get(local, ifsta->bssid);
	if (!sta) {
						...
						size_t len)
{
	...
	sta = rt2x_sta_info_get(local, mgmt->sa);
	if (!sta) {
		rcu_read_unlock();
		return;
	}
					     ...
					     size_t len)
{
	...
	if (le16_to_cpu(mgmt->u.action.u.addba_resp.status)
		...
		*state = HT_AGG_STATE_OPERATIONAL;
		spin_unlock_bh(&sta->ampdu_mlme.ampdu_tx);
		rt2x_ieee80211_stop_tx_ba_session(hw, sta->addr, tid,
					     WLAN_BACK_INITIATOR);
	}
			...
			struct ieee80211_mgmt *mgmt, size_t len)
{
	...
	if (initiator == WLAN_BACK_INITIATOR)
		...
		sta->ampdu_mlme.tid_state_tx[tid] =
				HT_AGG_STATE_OPERATIONAL;
		spin_unlock_bh(&sta->ampdu_mlme.ampdu_tx);
		rt2x_ieee80211_stop_tx_ba_session(&local->hw, sta->addr, tid,
					     WLAN_BACK_RECIPIENT);
	}
	rcu_read_unlock();
}
...
void sta_addba_resp_timer_expired(unsigned long data)
{
	...
	sta = rt2x_sta_info_get(local, temp_sta->addr);
	if (!sta) {
		rcu_read_unlock();
		return;
	}
	...
	*state = HT_AGG_STATE_OPERATIONAL;
	spin_unlock_bh(&sta->ampdu_mlme.ampdu_tx);
	rt2x_ieee80211_stop_tx_ba_session(hw, temp_sta->addr, tid,
				     WLAN_BACK_INITIATOR);
...
void ieee80211_sta_tear_down_BA_sessions(struct net_device *dev, u8 *addr)
{
	...
	for (i = 0; i <  STA_TID_NUM; i++) {
		rt2x_ieee80211_stop_tx_ba_session(&local->hw, addr, i,
					     WLAN_BACK_INITIATOR);
...
static void rt2x___ieee80211_rx_bss_hash_add(struct net_device *dev,
					struct ieee80211_sta_bss *bss)
{
	struct ieee80211_local *local = wdev_priv(dev->ieee80211_ptr);
	u8 hash_idx;
...
/* Caller must hold local->sta_bss_lock */
static void rt2x___ieee80211_rx_bss_hash_del(struct net_device *dev,
					struct ieee80211_sta_bss *bss)
{
	struct ieee80211_local *local = wdev_priv(dev->ieee80211_ptr);
		     ...
		     u8 *ssid, u8 ssid_len)
{
	...
	rt2x___ieee80211_rx_bss_hash_add(dev, bss);
				 ...
				 struct ieee80211_sta_bss *bss)
{
	...
	spin_lock_bh(&local->sta_bss_lock);
	rt2x___ieee80211_rx_bss_hash_del(dev, bss);
	list_del(&bss->list);
	spin_unlock_bh(&local->sta_bss_lock);
	...
	do {
		...
		*pos++ = rates;
		...
		if (bss->band == IEEE80211_BAND_2GHZ) {
			...
			*pos++ = 1;
			*pos++ = rt2x_ieee80211_frequency_to_channel(bss->freq);
		}
				  ...
				  int beacon)
{
	...
	if (elems.ds_params && elems.ds_params_len == 1)
		freq = rt2x_ieee80211_channel_to_frequency(elems.ds_params[0]);
	else
		freq = rx_status->freq;
	...
	if (sdata->vif.type == IEEE80211_IF_TYPE_IBSS && beacon &&
		...
		if (rx_status->flag & RX_FLAG_TSFT) {
			...
			rx_timestamp = -1LLU;
#ifdef LBM_CONFIG_MAC80211_IBSS_DEBUG
		printk(KERN_DEBUG "RX beacon SA=%s BSSID="
		       ...
		       jiffies);
#endif /* LBM_CONFIG_MAC80211_IBSS_DEBUG */
		if (beacon_timestamp > rx_timestamp) {
#ifndef LBM_CONFIG_MAC80211_IBSS_DEBUG
			if (net_ratelimit())
#endif
...
EXPORT_SYMBOL(rt2x_ieee80211_scan_completed);
...
void ieee80211_sta_scan_work(struct work_struct *work)
{
	...
	case SCAN_SET_CHANNEL:
		...
		if (!sband || local->scan_channel_idx >= sband->n_channels) {
			rt2x_ieee80211_scan_completed(local_to_hw(local));
			return;
		}
		skip = 0;
			  ...
			  char *current_ev, char *end_buf)
{
	...
	memset(&iwe, 0, sizeof(iwe));
	iwe.cmd = SIOCGIWFREQ;
	iwe.u.freq.m = rt2x_ieee80211_frequency_to_channel(bss->freq);
	iwe.u.freq.e = 0;
	current_ev = iwe_stream_add_event(current_ev, end_buf, &iwe,
					  IW_EV_FREQ_LEN);
