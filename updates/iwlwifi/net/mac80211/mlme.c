...
static struct ieee80211_quiet_info *
ieee80211_parse_quiet_elems(u8 *start, size_t len,
			    struct ieee802_11_elems *elems)
{
	size_t left = len;
	u8 *pos = start;
	struct ieee80211_quiet_info *quiet_info;
	int i = 0;

	quiet_info = kmalloc(sizeof(struct ieee80211_quiet_info) +
			     (elems->num_of_quiet_elem *
			      sizeof(struct ieee80211_quiet_ie)), GFP_KERNEL);
	quiet_info->num_of_quiet_ies = elems->num_of_quiet_elem;
	if (!quiet_info) {
		printk(KERN_ERR "allocation for quiet elements failed\n");
		return NULL;
	}

	/* TODO jump string to  elems->quiet_elem */
	/* fill buffer */
	while (left >= 2) {
		u8 id, elen;
		...
		if (id == WLAN_EID_QUIET &&
		    elen == sizeof(struct ieee80211_quiet_ie)) {
			memcpy(&quiet_info->ie[i], pos,
	...
	if (i != elems->num_of_quiet_elem) {
		printk(KERN_ERR "Quiet element count gone bad. Giving up.\n");
		kfree(quiet_info);
		quiet_info = NULL;
	}

	printk(KERN_DEBUG "Quiet element count successful.\n");
	return quiet_info;
}
...
/* returns 0 if no change was made, or BSS_CHANGED_PWR if new tpc
   txpower was defined */
...
/* returns 0 if no change was made, or BSS_CHANGED_PWR if new tpc
   txpower was defined */
static u32 ieee80211_sta_process_pwr_constraint(struct ieee80211_if_sta *ifsta,
						struct ieee80211_local *local,
						u8 *pwr_constr_ie)
{
	int power;
	u32 changed = 0;
	if (ifsta->country_max_pwr == 0) {
		printk(KERN_WARNING "country max power unknown. Unable to "
				" handle power constraint info element.\n");
		return 0;
	}
		
	power = ifsta->country_max_pwr - *pwr_constr_ie;
	if (local->hw.conf.tpc_pwr_limit != power)
		changed = BSS_CHANGED_PWR;
	local->hw.conf.tpc_pwr_limit = power;

#ifdef CONFIG_MAC80211_VERBOSE_SPECT_MGMT_DEBUG
	if (changed)
		printk(KERN_DEBUG "new max power is %d\n",
			local->hw.conf.tpc_pwr_limit);
	else
		 printk(KERN_DEBUG "no need to change tpc max power.\n");
#endif
	return changed;
}

static void ieee80211_sta_process_channel_switch(struct net_device *dev,
					struct ieee80211_if_sta *ifsta,
					struct ieee80211_channel_sw_ie *csa_ie)
{
	...
	freq = ieee80211_channel_to_frequency(csa_ie->new_ch_num);
	channel = ieee80211_get_channel(local->hw.wiphy, freq);

	if (!channel || channel->flags & IEEE80211_CHAN_DISABLED) {
#ifdef MAC80211_VERBOSE_SPECT_MGMT_DEBUG
		printk(KERN_DEBUG "can't find switch channel: %d\n",
		       csa_ie->new_ch_num);
#endif /* MAC80211_VERBOSE_SPECT_MGMT_DEBUG */
		return;
	}
	...
	ifsta->switch_channel = channel;
	ifsta->state = IEEE80211_STA_MLME_CHANNEL_SWITCH;
	mod_timer(&ifsta->timer, jiffies +
		  msecs_to_jiffies(csa_ie->count * local->hw.conf.beacon_int));
}
				     ...
				     bool assoc)
{
	...
	if (assoc) {
		...
		if (ifsta->ap_capab & WLAN_CAPABILITY_SPECTRUM_MGMT &&
		    local->hw.flags & IEEE80211_HW_SPECTRUM_MGMT)
			ifsta->flags |= IEEE80211_STA_SPECT_MGMT_ENABLED;
				 ...
				 struct ieee80211_if_sta *ifsta)
{
	...
	struct ieee80211_mgmt *mgmt;
	u8 *pos, *ies;
	int i, len, count, rates_len, supp_rates_len, listen_interval;
	u16 capab;
	struct ieee80211_sta_bss *bss;
	int wmm = 0;
	struct ieee80211_supported_band *sband;
	...
	listen_interval = (local->hw.conf.listen_interval != 0) ?
...
/**
 * process measurement requests: send report for basic
 * requests (mandatory by spec), and refuse all other
 * request types (optional by spec)
 */
static void ieee80211_sta_process_measurement_req(struct net_device *dev,
					struct ieee80211_mgmt *request_mgmt)
{
	struct ieee80211_local *local = wdev_priv(dev->ieee80211_ptr);
	struct sk_buff *skb;
	struct ieee80211_mgmt *report_mgmt;
	struct ieee80211_msrment_req_ie *request_ie =
		&request_mgmt->u.action.u.msrment_req.elem;
	size_t report_size =
		(request_ie->type == IEEE80211_SPCT_MSR_RPRT_TYPE_BASIC) ?
		sizeof(struct ieee80211_basic_msrment_rep) : 0;

	skb = dev_alloc_skb(sizeof(*report_mgmt)
				+ local->hw.extra_tx_headroom +	report_size);
	...
	skb_reserve(skb, local->hw.extra_tx_headroom);
	report_mgmt = (struct ieee80211_mgmt *)skb_put(skb, 24);
	memset(report_mgmt, 0, 24);
	memcpy(report_mgmt->da, request_mgmt->sa, ETH_ALEN);
	memcpy(report_mgmt->sa, dev->dev_addr, ETH_ALEN);
	memcpy(report_mgmt->bssid, request_mgmt->bssid, ETH_ALEN);
	report_mgmt->frame_control = IEEE80211_FC(IEEE80211_FTYPE_MGMT,
						IEEE80211_STYPE_ACTION);

	skb_put(skb, 1 + sizeof(report_mgmt->u.action.u.msrment_rep)
			+ report_size);
	report_mgmt->u.action.category = WLAN_CATEGORY_SPECTRUM_MGMT;
	report_mgmt->u.action.u.msrment_rep.action_code =
					WLAN_ACTION_SPCT_MSR_RPRT;
	report_mgmt->u.action.u.msrment_rep.dialog_token =
		request_mgmt->u.action.u.msrment_req.dialog_token;

	report_mgmt->u.action.u.msrment_rep.element_id =
					WLAN_EID_MEASURE_REPORT;
	report_mgmt->u.action.u.msrment_rep.length =
			sizeof(struct ieee80211_msrment_rep_ie)
			+ report_size;

	memset(&report_mgmt->u.action.u.msrment_rep.elem, 0,
		sizeof(struct ieee80211_msrment_rep_ie)	+ report_size);
	report_mgmt->u.action.u.msrment_rep.elem.token = request_ie->token;
	report_mgmt->u.action.u.msrment_rep.elem.type = request_ie->type;

	if (request_ie->type == IEEE80211_SPCT_MSR_RPRT_TYPE_BASIC) {
		struct ieee80211_basic_msrment_rep *basic_report =
			(struct ieee80211_basic_msrment_rep *)
			&report_mgmt->u.action.u.msrment_rep.elem.report;
		struct ieee80211_basic_msrment_req *basic_request =
			(struct ieee80211_basic_msrment_req *)
			&request_ie->request;

		basic_report->channel_number = basic_request->channel_number;
		basic_report->start_time = basic_request->start_time;
		basic_report->duration = basic_request->duration;
		basic_report->map |= IEEE80211_BASIC_MSR_REP_MAP_UNMEASURED;
	} else
		report_mgmt->u.action.u.msrment_rep.elem.mode |=
...
static void ieee80211_sta_process_tpc_req(struct net_device *dev,
					struct ieee80211_mgmt *tpc_request,
					struct ieee80211_rx_status *rx_status)
{
	...
	skb = dev_alloc_skb(sizeof(*tpc_report) + local->hw.extra_tx_headroom);
	...
	skb_reserve(skb, local->hw.extra_tx_headroom);
	tpc_report = (struct ieee80211_mgmt *)skb_put(skb, 24);
	memset(tpc_report, 0, 24);
	memcpy(tpc_report->da, tpc_request->sa, ETH_ALEN);
	memcpy(tpc_report->sa, dev->dev_addr, ETH_ALEN);
	memcpy(tpc_report->bssid, tpc_request->bssid, ETH_ALEN);
	tpc_report->frame_control = IEEE80211_FC(IEEE80211_FTYPE_MGMT,
						IEEE80211_STYPE_ACTION);

	skb_put(skb, 1 + sizeof(tpc_report->u.action.u.tpc_rep));
	tpc_report->u.action.category = WLAN_CATEGORY_SPECTRUM_MGMT;
	tpc_report->u.action.u.tpc_rep.action_code =
				WLAN_ACTION_SPCT_TPC_RPRT;
	tpc_report->u.action.u.tpc_rep.dialog_token =
				tpc_request->u.action.u.tpc_req.dialog_token;

	tpc_report->u.action.u.tpc_rep.element_id = WLAN_EID_TPC_REPORT;
	tpc_report->u.action.u.tpc_rep.length = 2;

	tpc_report->u.action.u.tpc_rep.txpower = local->hw.conf.power_level;
	tpc_report->u.action.u.tpc_rep.link_margin = rx_status->signal;
					 ...
					 int reassoc)
{
	...
	/* FIXME: Workaround - Until disassociation flow is trustworthy... */
...
/* returns 0 if no change was made, or BSS_CHANGED_PWR if new tpc txpower
   was defined */
static u32 ieee80211_handle_spect_mgmt_ies(struct net_device *dev,
					   struct ieee80211_sub_if_data *sdata,
					   u8 *start, size_t len,
					   struct ieee802_11_elems *elems)
{
	...
	if (elems->ch_switch_elem)
		ieee80211_sta_process_channel_switch(dev, ifsta,
			(struct ieee80211_channel_sw_ie *)
					elems->ch_switch_elem);

	if (elems->pwr_constr_elem)
		changed |= ieee80211_sta_process_pwr_constraint(ifsta, local,
							elems->pwr_constr_elem);
 	if (elems->num_of_quiet_elem &&
 	   (local->hw.flags & IEEE80211_HW_SPECTRUM_MGMT)) {
 		bss_conf->quiet_info = ieee80211_parse_quiet_elems(start,
 								  len, elems);
 		if (sdata->bss_conf.quiet_info)
 			changed |= BSS_CHANGED_QUIET;
 	}
				     ...
				     struct ieee80211_rx_status *rx_status)
{
	...
	/* 11h handling over A band (if supporting spectrum management) */
	if (ifsta->flags & IEEE80211_STA_SPECT_MGMT_ENABLED)
               changed |= ieee80211_handle_spect_mgmt_ies(dev, sdata,
					 ...
					 struct ieee80211_if_sta *ifsta)
{
	...
	printk(KERN_DEBUG "%s: switch to new channel %d\n", dev->name,
	       ieee80211_frequency_to_channel(
				ifsta->switch_channel->center_freq));
	...
	local->hw.conf.flags &= ~IEEE80211_CONF_CHANNEL_SWITCH;
	ieee80211_wake_queues(&local->hw);
...
void ieee80211_sta_work(struct work_struct *work)
{
	...
	case IEEE80211_STA_MLME_MESH_UP:
		...
		break;
#endif
	case IEEE80211_STA_MLME_CHANNEL_SWITCH:
			  ...
			  enum ieee80211_notification_types  notif_type)
{
	...
	case IEEE80211_NOTIFY_RE_ASSOC:
		...
		list_for_each_entry_rcu(sdata, &local->interfaces, list) {
			...
			/* Implemented for STA and IBSS only (so far) */
			if (sdata->vif.type != IEEE80211_IF_TYPE_STA &&
			    sdata->vif.type != IEEE80211_IF_TYPE_IBSS)
				continue;

			ieee80211_if_config(sdata->dev);

			change = ieee80211_reset_erp_info(sdata->dev);
			...
			case IEEE80211_IF_TYPE_IBSS:
				ifsta->ibss_join_req = jiffies;
				ifsta->state = IEEE80211_STA_MLME_IBSS_SEARCH;
				/* clear the "active" bssid, since we got
				 * disconnected */
