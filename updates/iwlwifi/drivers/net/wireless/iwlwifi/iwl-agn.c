...
static __le16 iwl_adjust_beacon_interval(u16 beacon)
{
	u16 new = 0;
	u16 beacon_factor = 0;

	beacon_factor = (beacon + MAX_UCODE_BEACON_INTERVAL) /
					MAX_UCODE_BEACON_INTERVAL;
	new = beacon / beacon_factor;

	return cpu_to_le16(new);
}
...
static void iwl_setup_rxon_timing(struct iwl_priv *priv)
{
	...
	intrval_tm = (u32)le16_to_cpu(priv->rxon_timing.beacon_interval) * 1024;
	tsf = priv->timestamp; /* tsf is modified by do_div: copy! */
	rem = do_div(tsf, intrval_tm);
	priv->rxon_timing.beacon_init_val = cpu_to_le32(intrval_tm - rem);

	IWL_DEBUG_ASSOC("beacon interval %d beacon timer %d beacon atim %d\n",
			le16_to_cpu(priv->rxon_timing.beacon_interval),
...
static int iwl4965_set_mode(struct iwl_priv *priv, int mode)
{
	...
	struct ieee80211_conf *conf = ieee80211_get_hw_conf(priv->hw);
...
static void iwl4965_irq_tasklet(struct iwl_priv *priv)
{
	...
	if (inta & CSR_INT_BIT_RF_KILL) {
		...
		/* driver only load ucode once setting the interface up.
		 * the driver as well wont allow loading if RFKILL is set
...
static int iwl4965_mac_config(struct ieee80211_hw *hw, struct ieee80211_conf *conf)
{
	...
	if (conf->flags & IEEE80211_CONF_CHANNEL_SWITCH) {
		priv->cfg->ops->lib->channel_switch(priv, channel);
		goto out;
	}
	...
	if (memcmp(&priv->active_rxon,
		...
		IWL_DEBUG_INFO("Not re-sending same RXON configuration.\n");
...
static int  iwl_disassociate(struct iwl_priv *priv)
{
	unsigned long flags;
				     ...
				     u32 changes)
{
	...
	if (changes & BSS_CHANGED_QUIET) {
		iwl_send_quiet(priv, bss_conf->quiet_info);
		kfree(bss_conf->quiet_info);
		bss_conf->quiet_info = NULL;
	}
...
static int iwl4965_pci_probe(struct pci_dev *pdev, const struct pci_device_id *ent)
{
	...
	/* Set the SW RF-kill. This is needed now and again in UP since the
	 * driver can be loaded with disable=1. In this case the RF-kill
	 * subsystem must be updated before UP since we fail the UP flow */
	iwl_rfkill_set_hw_state(priv);
