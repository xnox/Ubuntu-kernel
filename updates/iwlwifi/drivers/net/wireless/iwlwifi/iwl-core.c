...
void iwl_send_quiet(struct iwl_priv *priv,
		    struct ieee80211_quiet_info *quiet_info)
{
	struct iwl_quiet_cmd quiet_cmd;
	int i;
	u32 n_intervals;
	...
	memset(&quiet_cmd, 0, sizeof(quiet_cmd));

	if (quiet_info->num_of_quiet_ies > MAX_NUM_OF_QUIET_INTERVALS) {
		IWL_WARNING("Unsupported num of quiet intervals (%d). "
			    "Considering the first %d\n",
				quiet_info->num_of_quiet_ies,
				MAX_NUM_OF_QUIET_INTERVALS);
		n_intervals = MAX_NUM_OF_QUIET_INTERVALS;
	} else {
		n_intervals = quiet_info->num_of_quiet_ies;
	}

	IWL_DEBUG_11H("Quiet command: %d intervals:\n", n_intervals);

	for (i = 0; i < n_intervals; i++) {
		u32 start = (quiet_info->ie[i].count * priv->beacon_int)
			+ le16_to_cpu(quiet_info->ie[i].offset);
		u32 period = quiet_info->ie[i].period * priv->beacon_int;
		u32 duration = le16_to_cpu(quiet_info->ie[i].duration);
		u32 n_periods = 0xFFFFFFFF;
		IWL_DEBUG_11H("\tInterval %d: "
			"start: %d period %d num_of_periods %d duration %d\n",
			i, start, period, n_periods, duration);

		quiet_cmd.intervals[i].start = cpu_to_le32(start);
		quiet_cmd.intervals[i].period = cpu_to_le32(period);
		quiet_cmd.intervals[i].duration = cpu_to_le32(duration);
		quiet_cmd.intervals[i].num_of_periods = cpu_to_le32(n_periods);
	}
	quiet_cmd.num_of_intervals = cpu_to_le32(n_intervals);

	iwl_send_cmd(priv, &cmd);
}
EXPORT_SYMBOL(iwl_send_quiet);
...
void iwl_rf_kill(struct iwl_priv *priv)
{
	...
	if (!iwl_is_rfkill(priv)) {
		if (!test_bit(STATUS_EXIT_PENDING, &priv->status) &&
			...
			IWL_DEBUG(IWL_DL_RF_KILL,
			    "HW and/or SW RF Kill no longer active but "
			    "device is not up. Don't restart.\n");
		}
	} else {
		/* cast away the const for active_rxon in this function */
		struct iwl_rxon_cmd *active_rxon =
			(void *)&priv->active_rxon;
		/* make sure mac80211 stop sending Tx frame */
		if (priv->mac80211_registered)
			ieee80211_stop_queues(priv->hw);
...
int iwl_radio_kill_sw_enable_radio(struct iwl_priv *priv)
{
	 ...
	 */
	if (!iwl_is_alive(priv)) {
		IWL_DEBUG_RF_KILL("Enable radio: uCode not loaded, restart\n");
