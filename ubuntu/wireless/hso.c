 ...
 * Copyright (C) 2008 Greg Kroah-Hartman <gregkh[at]suse.de>
 ...
 * XXX We've commented out the ones that conflict with the option driver */
...
static int hso_net_start_xmit(struct sk_buff *skb, struct net_device *net)
{
	...
	usb_fill_bulk_urb(odev->mux_bulk_tx_urb,
			  ...
			  usb_sndbulkpipe(odev->parent->usb,
					  odev->
					  out_endp->bEndpointAddress & 0x7F),
...
static void hso_get_drvinfo(struct net_device *net,
	...
	while (count) {
		...
		case WAIT_IP:
			...
			temp_bytes =
			    (count <
			     odev->
			     rx_buf_missing) ? count : odev->rx_buf_missing;
			      ...
			      u8 * ctrl_urb_data, u32 size)
{
	...
	if (type == USB_CDC_GET_ENCAPSULATED_RESPONSE) {
		...
		ctrl_req->bRequestType = USB_DIR_IN |
		    USB_TYPE_OPTION_VENDOR | USB_RECIP_INTERFACE;
...
static struct hso_device *hso_create_bulk_serial_device(struct usb_interface
							*interface, int port)
{
	struct hso_device *hso_dev;
	struct hso_serial *serial;
		     ...
		     const struct usb_device_id *id)
{
	...
	case HSO_INTF_MUX:
		...
		for (i = 1, mux = 0; i < 0x100; i = i << 1, mux++) {
			if (port_mask & i) {
				hso_dev =
				    hso_create_mux_serial_device(interface, i,
								 shared_int);
				if (!hso_dev) {
					printk(" Failed %d\n", __LINE__);
					goto exit;
				   ...
				   struct usb_device *usb, gfp_t gfp)
{
	...
	usb_fill_int_urb(shared_int->shared_intr_urb, usb,
			 usb_rcvintpipe(usb,
					shared_int->intr_endp->
					bEndpointAddress & 0x7F),
			 shared_int->shared_intr_buf,
			 shared_int->intr_endp->wMaxPacketSize, intr_callback,
			 shared_int, shared_int->intr_endp->bInterval);
