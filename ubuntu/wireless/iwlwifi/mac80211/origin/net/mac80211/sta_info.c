...
static inline void __iwlwifi_sta_info_get(struct sta_info *sta)
{
	kref_get(&sta->kref);
}

struct sta_info *iwlwifi_sta_info_get(struct ieee80211_local *local, u8 *addr)
{
	struct sta_info *sta;
...
EXPORT_SYMBOL(iwlwifi_sta_info_get);
...
void iwlwifi_sta_info_put(struct sta_info *sta)
{
	kref_put(&sta->kref, sta_info_release);
}
EXPORT_SYMBOL(iwlwifi_sta_info_put);
			       ...
			       struct net_device *dev, u8 *addr, gfp_t gfp)
{
	...
	__iwlwifi_sta_info_get(sta);	/* sta used by caller, decremented by
