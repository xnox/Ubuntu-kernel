...
static void rt2x00lib_rfkill_poll(struct input_polled_dev *poll_dev)
{
...
int rt2x00lib_register_rfkill(struct rt2x00_dev *rt2x00dev)
{
	...
	status = rfkill_register(rt2x00dev->rfkill);
	if (status) {
		ERROR(rt2x00dev, "Failed to register rfkill handler.\n");
		return status;
	}

	status = input_register_polled_device(rt2x00dev->poll_dev);
	if (status) {
...
int rt2x00lib_allocate_rfkill(struct rt2x00_dev *rt2x00dev)
{
	...
	rt2x00dev->rfkill->toggle_radio = rt2x00lib_toggle_radio;
	...
	rt2x00dev->poll_dev->poll = rt2x00lib_rfkill_poll;
