...
static void * rtl_ieee80211_tkip_init(int key_idx)
{
	...
	priv->tfm_arc4 = crypto_alloc_blkcipher("arc4", 0, CRYPTO_ALG_ASYNC);
		       ...
		       u8 *data, size_t data_len, u8 *mic)
{
	...
	struct hash_desc desc = { .tfm = tkey->tfm_michael };
	...
	crypto_hash_setkey(tkey->tfm_michael, key, 8);
	crypto_hash_update(&desc, sg, 2);
	crypto_hash_final(&desc, mic);
