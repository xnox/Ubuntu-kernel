	...
	IN PSTRING arg)
{
	...
	DBGPRINT(RT_DEBUG_OFF, ("Mesh Proxy Mac Hash Table \n"));
	for(i=0;i<MAX_HASH_ENTRY_TAB_SIZE;i++)
	{
		if (pProxyTab->Hash[i]!=-1)
			DBGPRINT(RT_DEBUG_OFF, ("%03d : %d \n",i,pProxyTab->Hash[i]));
	...
	if (pAd->MeshTab.MeshLink[idx].Entry.bValidLocalMpAsSelector)
	{
		...
		switch (AuthMode)
        {
           	case Ndis802_11AuthModeWPA2:            	
               	NdisMoveMemory(pLocalMsaIe->SelectedAKM, OUI_MSA_8021X_AKM, OUI_SUITE_LEN);
            break;

          	case Ndis802_11AuthModeWPA2PSK:
           		NdisMoveMemory(pLocalMsaIe->SelectedAKM, OUI_MSA_PSK_AKM, OUI_SUITE_LEN);
            break;
...
#define _TX_RATE_TAB_SIZE (sizeof(TxRate) /  sizeof(_TX_RATE))
	...
	for (loopIdx = 0; loopIdx < _TX_RATE_TAB_SIZE; loopIdx++)
	{
			...
			&& (TxRate[loopIdx].MCS == HTTxMode.field.MCS))
		{
			...
			if (pMacEntry != NULL)
				ErrRate = pMacEntry->PER[TxRate[loopIdx].RateIdx];
			else
				ErrRate = 0;
			break;
		}
