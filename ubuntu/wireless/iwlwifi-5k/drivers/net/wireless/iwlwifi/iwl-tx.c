...
int iwl_tx_queue_reclaim(struct iwl_priv *priv, int txq_id, int index)
{
	...
	for (index = iwl_queue_inc_wrap(index, q->n_bd); q->read_ptr != index;
		...
		if (tx_info->skb[0] == NULL) {
			IWL_ERROR("Reclaiming already freed buffer.\n");
			continue;
		}
...

{
	...
	if (iwl_queue_used(&priv->txq[scd_flow].q, agg->start_idx) == 0) {
		IWL_ERROR("Wrong scd pointer txq range [%d - %d], start_idx %d",
			priv->txq[scd_flow].q.read_ptr,
			priv->txq[scd_flow].q.write_ptr, agg->start_idx);
		return 0;
	}
