...
static void iwl5000_fill_txpower_tbl(struct iwl5000_tx_power_db *tx_power_tbl)
{
	int i;

	for (i = 0; i < 24; i++) {
		tx_power_tbl->ht_ofdm_power[i].s.radio_tx_gain[0] = 0x16;
		tx_power_tbl->ht_ofdm_power[i].s.radio_tx_gain[1] = 0x16;
		tx_power_tbl->ht_ofdm_power[i].s.radio_tx_gain[2] = 0x16;
		tx_power_tbl->ht_ofdm_power[i].s.dsp_predis_atten[0] = 0x6e;
		tx_power_tbl->ht_ofdm_power[i].s.dsp_predis_atten[1] = 0x6e;
		tx_power_tbl->ht_ofdm_power[i].s.dsp_predis_atten[2] = 0x6e;
	}

	for (i = 0; i < 2; i++) {
		tx_power_tbl->cck_power[i].s.radio_tx_gain[0] = 0x16;
		tx_power_tbl->cck_power[i].s.radio_tx_gain[1] = 0x16;
		tx_power_tbl->cck_power[i].s.radio_tx_gain[2] = 0x16;
		tx_power_tbl->cck_power[i].s.dsp_predis_atten[0] = 0x6e;
		tx_power_tbl->cck_power[i].s.dsp_predis_atten[1] = 0x6e;
		tx_power_tbl->cck_power[i].s.dsp_predis_atten[2] = 0x6e;
	}
}
...
static int iwl5000_hw_channel_switch(struct iwl_priv *priv, u16 channel)
{
	...
	iwl5000_fill_txpower_tbl(&cmd.tx_power);

	ret = iwl_send_cmd_pdu(priv, REPLY_CHANNEL_SWITCH, sizeof(cmd), &cmd);
	if (ret)
		IWL_ERROR("Bad return from REPLY_CHANNEL_SWITCH command\n");
				      ...
				      int txq_id, u16 start_idx)
{
	...
	if (agg->frame_count == 1) {
		...
		for (i = 0; i < agg->frame_count; i++) {
			...
			if (iwl_queue_used(&priv->txq[txq_id].q, idx) == 0) {
				IWL_ERROR("Wrong scd pointer,"
					  "txq range [%d - %d], idx %d",
					priv->txq[txq_id].q.read_ptr,
					priv->txq[txq_id].q.write_ptr, idx);
				continue;
			}
...
static struct iwl_lib_ops iwl5000_lib = {
	...
	.channel_switch = iwl5000_hw_channel_switch,
	.apm_ops = {
