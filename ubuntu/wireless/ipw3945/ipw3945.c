...
static void ipw_clear_free_frames(struct ipw_priv *priv)
{
	...
	IPW_DEBUG_INFO("%d frame[s] on pre-allocated heap on clear.\n",
		       priv->frames_count);
...
static void ipw_bg_daemon_cmd(struct work_struct *work);
			 ...
			 const char *buf, size_t count)
{
	...
	queue_delayed_work(priv->daemonqueue, &priv->daemon_cmd_work, 0);
...
static int ipw_start_association(struct ipw_priv *priv,
				 struct ipw_associate *associate)
{
	int rc = 0;
	struct daemon_rx_config *rxon = &priv->rxon;
	...
	if (!priv->assoc_sequence_frame)
		priv->assoc_sequence_frame = ipw_get_free_frame(priv);
	if (!priv->assoc_sequence_frame) {
		IPW_ERROR("Could not obtain frame for auth work.\n");
		return -ENOMEM;
	}

	auth_frame = (void *)&priv->assoc_sequence_frame->u.frame;
	priv->assoc_sequence_frame->len = sizeof(*auth_frame);

	auth_frame->header.frame_ctl =
	    IEEE80211_FTYPE_MGMT | IEEE80211_STYPE_AUTH;
	auth_frame->transaction = 1;

	auth_frame->algorithm = (associate->auth_type == AUTH_SHARED_KEY) ?
	    WLAN_AUTH_SHARED_KEY : WLAN_AUTH_OPEN;

	memcpy(auth_frame->header.addr1, priv->bssid, ETH_ALEN);
	memcpy(auth_frame->header.addr2, priv->mac_addr, ETH_ALEN);
	memcpy(auth_frame->header.addr3, priv->bssid, ETH_ALEN);
	...
	if (rxon->dev_type == RXON_DEV_TYPE_IBSS) {
		...
		if (ipw_rxon_add_station(priv, priv->bssid, 1) ==
			...
			rc = ieee80211_tx_frame(priv->ieee,
						...
						len);
#endif
			priv->auth_state = AUTH_TRANSACTION_1;
			spin_unlock_irqrestore(&priv->lock, flags);
			priv->association_retries = 0;
			queue_delayed_work(priv->workqueue,
					   &priv->assoc_state_retry,
					   msecs_to_jiffies
					   (IPW_ASSOC_STATE_TIMEOUT));
		}
	}
...
static void ipw_bg_calibrated_work(struct work_struct *work)
{
	struct ipw_priv *priv =
	    container_of(work, struct ipw_priv, calibrated_work);
	int rc = 0;
	mutex_lock(&priv->mutex);
...
static u8 ipw_remove_station(struct ipw_priv *priv, u8 * bssid, int is_ap)
{
	...
	if (is_ap) {
		...
		for (i = STA_ID; i < STA_ID + min(priv->num_stations,
						  (u8) (NUM_OF_STATIONS - 3));
		     i++) {
...
static int ipw_disassociate(struct ipw_priv *priv)
{
				  ...
				  int roaming)
{
	...
	if (!memcmp(network->bssid, priv->bssid, ETH_ALEN)) {
		...
		IPW_DEBUG_MERGE("Network '%s (" "%s" ")' excluded "
				...
				escape_essid(network->ssid,
					     network->ssid_len),
				print_mac(mac1, network->bssid), print_mac(mac2, priv->bssid));
		return 0;
	}
...
static void ipw_bg_request_scan(struct work_struct *work)
{
	...
	if (scan->flags & DAEMON_SCAN_FLAG_ACTIVE) {
		if (scan->flags & DAEMON_SCAN_FLAG_DIRECT)
			IPW_DEBUG_SCAN("Initiating direct scan for %s.\n",
				       escape_essid(scan->direct_scan.ssid,
						    scan->direct_scan.len));
		else
	...
	element->cmd.data_len = sizeof(*scan) + scan->probe_request_len +
...
static void ipw_send_association_req(struct ipw_priv *priv)
{
	unsigned long flags;

	priv->assoc_sequence_frame->len =
	    ipw_fill_association_req(priv, &priv->assoc_sequence_frame->u.frame,
				     IEEE80211_FRAME_LEN);

	IPW_DEBUG_INFO("Sending association request frame (%d bytes).\n",
		       priv->assoc_sequence_frame->len);

	spin_lock_irqsave(&priv->lock, flags);
#if IPW3945_COMPAT >= 2
	ieee80211_tx_frame(priv->ieee, &priv->assoc_sequence_frame->u.frame, 0,
			   priv->assoc_sequence_frame->len, 0);
#else
	ieee80211_tx_frame(priv->ieee, &priv->assoc_sequence_frame->u.frame,
			   priv->assoc_sequence_frame->len);
#endif
	spin_unlock_irqrestore(&priv->lock, flags);

	priv->association_retries = 0;
	queue_delayed_work(priv->workqueue, &priv->assoc_state_retry,
			   msecs_to_jiffies(IPW_ASSOC_STATE_TIMEOUT));
}

static void ipw_bg_auth_work(struct work_struct *work)
{
	struct ipw_priv *priv = container_of(work, struct ipw_priv, auth_work);
	...
	auth = (void *)&priv->assoc_sequence_frame->u.frame;

	if (auth->algorithm == WLAN_AUTH_OPEN) {
		if (priv->auth_state != AUTH_TRANSACTION_1) {
			IPW_DEBUG_ASSOC
			    ("Invalid AP AUTH_OPEN failed reason: %d\n",
			     auth->status);
		}
		...
		IPW_DEBUG_ASSOC("AUTH_OPEN failed reason: %d\n", auth->status);
	...
	if (auth->algorithm != WLAN_AUTH_SHARED_KEY ||
	    priv->auth_state == AUTH_INIT) {
		IPW_DEBUG_ASSOC
		    ("Out of lock for auth sequence/algorithm: %d:%d\n",
		     priv->auth_state, auth->algorithm);
	...
	case 2:
		{
			...
			int tx_key = !(priv->ieee->sec.flags & SEC_ACTIVE_KEY) ?
			    0 : priv->ieee->sec.active_key;
			struct ieee80211_crypt_data *crypt =
			...
			if (priv->auth_state != AUTH_TRANSACTION_1) {
				IPW_DEBUG_ASSOC
				    ("Duplicate frame received for AUTH 1\n");
				break;
			}
#if IPW3945_COMPAT >= 2
			auth->header.seq_ctl = 0;

			memcpy(auth->header.addr1, priv->bssid, 6);
			memcpy(auth->header.addr2, priv->mac_addr, 6);
			memcpy(auth->header.addr3, priv->ieee->bssid, 6);
			auth->algorithm = WLAN_AUTH_SHARED_KEY;
			auth->transaction = 3;
			auth->status = 0;
			priv->assoc_sequence_frame->len = sizeof(*auth) + 130;

			ieee80211_tx_frame(priv->ieee,
					   &priv->assoc_sequence_frame->u.frame,
					   IEEE80211_3ADDR_LEN,
					   priv->assoc_sequence_frame->len, 1);
			...
			skb_auth = dev_alloc_skb(sizeof(struct ieee80211_auth)
						 + 138);
			if (!skb_auth) {
			...
			memcpy(auth, skb_auth->data, skb_auth->len);

			priv->assoc_sequence_frame->len = skb_auth->len;

			ieee80211_tx_frame(priv->ieee,
					   (struct ieee80211_hdr *)auth_frame,
					   priv->assoc_sequence_frame->len);
			...
			queue_delayed_work(priv->workqueue,
					   ...
					   (IPW_ASSOC_STATE_TIMEOUT));
#endif
			priv->auth_state = AUTH_TRANSACTION_3;
			break;
		}

	case 4:
		if (priv->auth_state != AUTH_TRANSACTION_3) {
			IPW_DEBUG_ASSOC
			    ("Duplicate frame received for AUTH 3\n");
			break;
		}
		...
		priv->auth_state = AUTH_INIT;
		IPW_DEBUG_ASSOC("AUTH_SHARED_KEY fail reason: %d\n",
				auth->status);
		break;

	default:
		IPW_DEBUG_ASSOC("Unhandled auth transaction state: %d\n",
				auth->transaction);
	}
	...
	return;
      ...
      authenticated:
	priv->auth_state = AUTH_SUCCESS;
	spin_unlock_irqrestore(&priv->lock, flags);
	if (!(priv->status & STATUS_ASSOCIATED))
		ipw_send_association_req(priv);
	mutex_unlock(&priv->mutex);
	return;
				 ...
				 u32 rates_mask, int roaming)
{
	...
	err = ipw_start_association(priv, &priv->assoc_request);
	if (err) {
...
static void ipw_roam(struct ipw_priv *priv)
{
	struct ieee80211_network *network = NULL;
	struct ipw_network_match match = {
		.network = priv->assoc_network
...
{
	struct ipw_priv *priv = container_of(work, struct ipw_priv, roam);
...
static void ipw_bg_daemon_tx_status_sync(struct work_struct *work)
{
	struct ipw_priv *priv =
	    container_of(work, struct ipw_priv, daemon_tx_status_sync);
...
{
	struct ipw_priv *priv = container_of(work, struct ipw_priv, associate);
...
#define IPW_PACKET_RETRY_TIME (5 * HZ)
...
static void ipw_bg_rate_scale_flush(struct work_struct *work)
{
	struct ipw_priv *priv = IPW_DELAYED_DATA(work, struct ipw_priv,
						 rate_scale_flush);
	int rc = 0;
					 ...
					 struct ipw_tx_resp *tx_resp)
{
	...
	if (time_after(jiffies, priv->rate_scale_mgr.stamp +
		       priv->rate_scale_mgr.flush_time)) {
		cancel_delayed_work(&priv->rate_scale_flush);
		queue_delayed_work(priv->workqueue, &priv->rate_scale_flush, 0);
		priv->rate_scale_mgr.stamp = jiffies;
...
/*
 * This callback is used for retransmitting the current auth/assoc frame
 * during an association sequence
 */
static void ipw_bg_assoc_state_retry(struct work_struct *work)
{
	unsigned long flags;
	struct ipw_priv *priv = IPW_DELAYED_DATA(work, struct ipw_priv,
						 assoc_state_retry);
	...
	if (priv->association_retries++ ==
	    ((priv->data_retry_limit == -1) ? IPW_MAX_ASSOC_RETRY :
	     priv->data_retry_limit)) {
		IPW_DEBUG_ASSOC("Association failed after %d checks.\n",
				priv->association_retries);
		ipw_send_disassociate(priv, 1);
		goto unlock;
	}

	priv->assoc_sequence_frame->u.frame.frame_ctl |= IEEE80211_FCTL_RETRY;

	IPW_DEBUG_INFO("Sending auth/assoc frame at state %d (%d bytes)\n",
		       priv->auth_state, priv->assoc_sequence_frame->len);
	printk_buf(IPW_DL_INFO,
		   (void *)&priv->assoc_sequence_frame->u.frame,
		   priv->assoc_sequence_frame->len);
...
#if IPW3945_COMPAT >= 2
	ieee80211_tx_frame(priv->ieee,
			   &priv->assoc_sequence_frame->u.frame, 0,
			   priv->assoc_sequence_frame->len, 0);
#else
	ieee80211_tx_frame(priv->ieee,
			   &priv->assoc_sequence_frame->u.frame,
...
static int ipw_setup_deferred_work(struct ipw_priv *priv)
{
	...
	IPW_INIT_WORK(&priv->associate, ipw_bg_associate);
	IPW_INIT_WORK(&priv->calibrated_work, ipw_bg_calibrated_work);
	IPW_INIT_WORK(&priv->disassociate, ipw_bg_disassociate);
	IPW_INIT_WORK(&priv->rx_replenish, ipw_bg_rx_queue_replenish);
	IPW_INIT_DELAYED_WORK(&priv->rf_kill, ipw_bg_rf_kill);
	IPW_INIT_DELAYED_WORK(&priv->up, ipw_bg_up);
	IPW_INIT_DELAYED_WORK(&priv->down, ipw_bg_down);
	IPW_INIT_DELAYED_WORK(&priv->request_scan, ipw_bg_request_scan);
	IPW_INIT_DELAYED_WORK(&priv->gather_stats, ipw_bg_gather_stats);
	IPW_INIT_WORK(&priv->abort_scan, ipw_bg_abort_scan);
	IPW_INIT_WORK(&priv->roam, ipw_bg_roam);
	IPW_INIT_DELAYED_WORK(&priv->scan_check, ipw_bg_scan_check);
	IPW_INIT_WORK(&priv->link_up, ipw_bg_link_up);
	IPW_INIT_WORK(&priv->update_link_led, ipw_bg_update_link_led);
	IPW_INIT_WORK(&priv->link_down, ipw_bg_link_down);
	IPW_INIT_WORK(&priv->auth_work, ipw_bg_auth_work);
	IPW_INIT_WORK(&priv->merge_networks, ipw_merge_adhoc_network);
	IPW_INIT_DELAYED_WORK(&priv->assoc_state_retry,
...
#ifdef CONFIG_IPW3945_QOS
	/* QoS */
	IPW_INIT_WORK(&priv->qos_activate, ipw_bg_qos_activate);
#endif

	/* 802.11h */
	IPW_INIT_DELAYED_WORK(&priv->report_work, ipw_bg_report_work);

	IPW_INIT_WORK(&priv->post_associate, ipw_bg_post_associate);

	IPW_INIT_DELAYED_WORK(&priv->activity_timer, ipw_bg_activity_timer);

	IPW_INIT_DELAYED_WORK(&priv->daemon_cmd_work, ipw_bg_daemon_cmd);
	IPW_INIT_WORK(&priv->daemon_tx_status_sync,
		      ipw_bg_daemon_tx_status_sync);

	IPW_INIT_DELAYED_WORK(&priv->rate_scale_flush, ipw_bg_rate_scale_flush);
...
static void ipw_bg_down(struct work_struct *work)
{
	struct ipw_priv *priv = IPW_DELAYED_DATA(work, struct ipw_priv, down);
