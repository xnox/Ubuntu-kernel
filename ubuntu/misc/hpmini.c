...
#if HP_MINI_DEBUG
#define DPRINT(format, args...) printk(DRIVER_PREFIX format, ##args)
#else
...
static acpi_status set_network_device_state(u32 network_device_state)
{
	...
	status = wmi_evaluate_method(
			HP_WMI_READ_WRITE_DATA_GUID,		/* GUID */
			1,					/* instance */
...
static ssize_t bluetooth_show(struct device *dev, struct device_attribute *devattr, char *buf)
{
	...
	return snprintf(buf, PAGE_SIZE, "%u\n",
		(network_device_state &
		 HP_WMI_NETWORK_STATE_BLUETOOTH_ON) ? 1 : 0);
}


static ssize_t bluetooth_set(struct device *dev,
...
static ssize_t wifi_set(struct device *dev,
			struct device_attribute *devattr,
			const char *buf, size_t count)
{
	unsigned long val;
...
static int __init hp_mini_init_module(void)
{
	...
	printk(KERN_INFO DRIVER_PREFIX "module loaded: v%s (%s)\n",
	       DRIVER_VERSION,
	       DRIVER_RELEASE_DATE);
...
static void __exit hp_mini_cleanup_module(void)
{
	...
	platform_device_unregister(hpmini_platform_device);
	platform_driver_unregister(&hpmini_driver);
	printk(KERN_INFO DRIVER_PREFIX "module unloaded\n");
}
