	...
	do {
		...
		if (msb->seg_count) {
			...
			param = (struct mspro_param_register) {
				...
				/* Note that this cast is a hack to get it to
				   build on Ubuntu - I suspect that we're fine
				   given the maximum size of MS right now */
				.data_address = cpu_to_be32((int)req->sector
							    / (msb->page_size
							       >> 9)),
				.cmd_param = 0
			};
