...
static u32 rtl8101_vpd_read(struct net_device *dev, unsigned int read_addr);
static void rtl8101_vpd_write(struct net_device *dev, unsigned int write_addr, u32 write_data);
static int rtl8101_eeprom_size(struct net_device *dev);
static u32 rtl8101_eeprom_read_dword(struct net_device *dev, unsigned int read_addr);
static void rtl8101_eeprom_write_dword(struct net_device *dev, u32 eeprom_cont, unsigned int write_addr);
static int rtl8101_eeprom_read(struct net_device *dev, u32 *eeprom_cont, int eeprom_size);
static void rtl8101_eeprom_write(struct net_device *dev, u32 *eeprom_cont, int eeprom_size);
void rtl8101_set_loopback(struct net_device *dev, int type, int state);
int rtl8101_loopback_test(struct net_device *dev);
static int rtl8101_interrupt_test(struct net_device *dev, u32 *isr_content);
static int rtl8101_report_device_type(struct net_device *dev, char *device_type);
static int rtl8101_report_device_id(struct net_device *dev, char *device_id);
int rtl8101_priv_ioctl(struct net_device *dev, struct ifreq *rq, int cmd);

static u32
rtl8101_vpd_read(struct net_device *dev, unsigned int read_addr)
{
...
static void
rtl8101_vpd_write(struct net_device *dev, unsigned int write_addr, u32 write_data)
{
	struct rtl8101_private *tp = netdev_priv(dev);
	struct pci_dev *pdev = tp->pci_dev;
	int vpd_addr = VPD_ADDRESS;
...
static int
rtl8101_eeprom_size(struct net_device *dev)
{
	struct rtl8101_private *tp = netdev_priv(dev);
	...
	if (RTL_R32(RxConfig) & EEPROM_9356) {
		offset_0x00 = rtl8101_eeprom_read_dword(dev, 0x00);
		offset_0x100 = rtl8101_eeprom_read_dword(dev, 0x100);

		rtl8101_eeprom_write_dword(dev, 0x56565656, 0x8000);
		rtl8101_eeprom_write_dword(dev, 0x66666666, 0x8100);

		test_cont1 = rtl8101_eeprom_read_dword(dev, 0x00);
		printk("test_cont1: %X\n", test_cont1);
		...
		rtl8101_eeprom_write_dword(dev, offset_0x00, 0x8000);
		rtl8101_eeprom_write_dword(dev, offset_0x100, 0x8100);
	} else {
...
static u32
rtl8101_eeprom_read_dword(struct net_device *dev, unsigned int read_addr)
{
	return rtl8101_vpd_read(dev, read_addr);
}

static void
rtl8101_eeprom_write_dword(struct net_device *dev, u32 eeprom_cont, unsigned int write_addr)
{
	rtl8101_vpd_write(dev, write_addr, eeprom_cont);
}


static int
rtl8101_eeprom_read(struct net_device *dev, u32 *eeprom_cont, int eeprom_size)
{
	int i;
	unsigned int read_addr;

	for (i = 0, read_addr = 0; i < eeprom_size / 4; i++)
		*(eeprom_cont + i) = rtl8101_vpd_read(dev, read_addr + i * 4);

	return 0;
}

static void
rtl8101_eeprom_write(struct net_device *dev, u32 *eeprom_cont, int eeprom_size)
{
	int i;
	unsigned int write_addr = 0x00;

	for (i = 0; i < eeprom_size / 4; i++)
		rtl8101_vpd_write(dev, write_addr + i * 4, *(eeprom_cont + i));
}

void 
rtl8101_set_loopback(struct net_device *dev, int type, int state)
{
	struct rtl8101_private *tp = netdev_priv(dev);
	void __iomem *ioaddr = tp->mmio_addr;
	int i;
...
int 
rtl8101_loopback_test(struct net_device *dev)
{
	struct rtl8101_private *tp = netdev_priv(dev);
...
static int 
rtl8101_interrupt_test(struct net_device *dev, u32 *isr_content)
{
	struct rtl8101_private *tp = netdev_priv(dev);
	void __iomem *ioaddr = tp->mmio_addr;
...
static int 
rtl8101_report_device_type(struct net_device *dev, char *device_type)
{
	strcpy(device_type, "PCI-E Fast Ethernet");

	return 0;
}

static int 
rtl8101_report_device_id(struct net_device *dev, char *device_id)
{
	strcpy(device_id, "8101");
...
int 
rtl8101_priv_ioctl(struct net_device *dev, struct ifreq *rq, int cmd)
{
	struct rtl8101_private *tp = netdev_priv(dev);
	void __iomem *ioaddr = tp->mmio_addr;
	...
	case RTL_EEPROM_SIZE:
		eeprom_size = rtl8101_eeprom_size(dev);
	...
	case RTL_READ_EEPROM:
		...
		rtl8101_eeprom_read(dev, eeprom_cont, eeprom_size);
	...
	case RTL_WRITE_EEPROM:
		...
		rtl8101_eeprom_write(dev, eeprom_cont, eeprom_size);
		kfree(eeprom_cont);
		return 0;
	case RTL_LOOPBACK:
		...
		//if do loopback, copy the payload from user sapce
		...
		rtl8101_set_loopback(dev, loopback_test.type, loopback_test.state);
		...
		return 0;
	case RTL_INTERRUPT:
		rtl8101_interrupt_test(dev, &isr_content);
	...
	case RTL_DEVICE_TYPE:
		...
		rtl8101_report_device_type(dev, device_type);
	...
	case RTL_DEVICE_ID:
		...
		rtl8101_report_device_id(dev, device_id);
