...
static const struct {
	...
	_R("RTL8102E", CFG_METHOD_3, 0xff7e1880),		
	_R("RTL8102E", CFG_METHOD_4, 0xff7e1880),		
	_R("RTL8102E", CFG_METHOD_5, 0xff7e1880),		
	_R("RTL8101E", CFG_METHOD_6, 0xff7e1880),	
};
#undef _R
...
#define BMCR_SPEED10	0x0000
		...
		if ((speed == SPEED_100) && (tp->mcfg == CFG_METHOD_3)) {
			spin_lock_irqsave(&tp->phy_lock, flags);
			mdio_write(ioaddr, 0x1f, 0x0000);
			mdio_write(ioaddr, 0x00, 0x2100);
			mdio_write(ioaddr, 0x1f, 0x0001);
			mdio_write(ioaddr, 0x19, 0x1f01);
			mdio_write(ioaddr, 0x1f, 0x0000);
			spin_unlock_irqrestore(&tp->phy_lock, flags);
		}
		    ...
		    struct sk_buff *skb)
{
	...
	if (tp->vlgrp && (opts2 & RxVlanTag)) {
		rtl8101_rx_hwaccel_skb(skb, tp->vlgrp,
				       swab16(opts2 & 0xffff));
		ret = 0;
	} else
		ret = -1;
...
rtl8101_hw_phy_config(struct net_device *dev)
{
	...
	if (tp->mcfg == CFG_METHOD_3) {
		...
		mdio_write(ioaddr, 0x07, 0xa17b);
		mdio_write(ioaddr, 0x04, 0xc066);
		mdio_write(ioaddr, 0x1f, 0x0000);
		 ...
		 int cmd)
{
	...
	default:
#ifdef RTL_IOCTL_SUPPORT
		return rtl8101_priv_ioctl(dev, ifr, cmd);
#else
		return -EOPNOTSUPP;		
#endif
	}
...
#ifdef RTL_IOCTL_SUPPORT
	tasklet_init(&tp->loopback_tasklet, rtl8101_loopback_test, (unsigned long)dev);
#endif 
...
#else
	retval = request_irq(dev->irq, rtl8101_interrupt, IRQF_SHARED, dev->name, dev);
#endif
	if (retval < 0)
		goto out;
...
rtl8101_hw_start(struct net_device *dev)
{
	...
	if (tp->mcfg == CFG_METHOD_3) {
		...
		RTL_W16(RxMaxSize, 0x05EF);

		RTL_W8(Config3, RTL_R8(Config3) & ~Beacon_en);

		data_u8 = RTL_R8(Config1);
		if ((data_u8 & LEDS0) && (data_u8 & LEDS1))
		...
		//E-PHY configuration
		rtl8101_ephy_write(ioaddr, 0x01, 0x6e65);
		rtl8101_ephy_write(ioaddr, 0x02, 0x091f);
		rtl8101_ephy_write(ioaddr, 0x03, 0xc2f9);
		rtl8101_ephy_write(ioaddr, 0x06, 0xafb5);
		rtl8101_ephy_write(ioaddr, 0x07, 0x0e00);
		rtl8101_ephy_write(ioaddr, 0x19, 0xec80);
		rtl8101_ephy_write(ioaddr, 0x01, 0x2e65);
		rtl8101_ephy_write(ioaddr, 0x01, 0x6e65);
...
#ifdef	RTL_IOCTL_SUPPORT
static int 
rtl8101_loopback_check_pkt(struct sk_buff *skb,
			   struct net_device *dev)
{
	struct rtl8101_private *tp = netdev_priv(dev);
	int i;
		   ...
		   struct net_device *dev)
{
	...
	if (tp->loopback_state == LOOPBACK_ON) {
		netif_stop_queue(dev);
		if (rtl8101_loopback_check_pkt(skb, dev))
...
#ifdef	RTL_IOCTL_SUPPORT
			if (tp->loopback_state == LOOPBACK_ON) {
				if (rtl8101_loopback_check_pkt(tp->Rx_skbuff[entry], dev))
					tp->loopback_rx_err++;
				else
					tp->loopback_rx_ok++;
...
rtl8101_close(struct net_device *dev)
{
	...
	if (tp->loopback_state == LOOPBACK_ON) {
		...
		udelay(1000);
		rtl8101_set_loopback(dev, tp->loopback_type, tp->loopback_state);
	}
