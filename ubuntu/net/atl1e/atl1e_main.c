...
char atl1e_driver_version[] = "1.0.0.1";
...
#else
static int atl1e_notify_reboot(struct notifier_block *nb, unsigned long event, void *p);

struct notifier_block atl1e_notifier_reboot = {
    .notifier_call  = atl1e_notify_reboot,
    .next       = NULL,
    .priority   = 0
};
#endif
...
static int
atl1e_notify_reboot(struct notifier_block *nb, unsigned long event, void *p)
{
    struct pci_dev *pdev = NULL;

    DEBUGFUNC("atl1e_notify_reboot !");
    ...
    case SYS_POWER_OFF:
        while((pdev = pci_find_device(PCI_ANY_ID, PCI_ANY_ID, pdev))) {
            if(pci_dev_driver(pdev) == &atl1e_driver)
