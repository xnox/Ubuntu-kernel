...
static int atl1c_setup_mac_funcs(struct atl1c_hw *hw)
{
	 ...
	 */
//	hw->ctrl_flags |= ATL1C_ASPM_L1_SUPPORT | ATL1C_ASPM_L0S_SUPPORT;
...
static int __devinit atl1c_sw_init(struct atl1c_adapter *adapter)
{
	...
	hw->base_cpu = min(AT_MAX_RECEIVE_QUEUE, num_online_cpus());
...
static int atl1c_alloc_rx_buffer(struct atl1c_adapter *adapter, const int ringid)
{
	...
	while (next_info->state == ATL1_BUFFER_FREE) {
		if (buffer_info->skb) {
			/* FIXME: just for test, should be removed */
			printk(KERN_EMERG"should not be happend!\n");
			dump_stack();
		}
...
{
	struct net_device *netdev = pci_get_drvdata(pdev);
	struct atl1c_adapter *adapter = netdev->priv;
